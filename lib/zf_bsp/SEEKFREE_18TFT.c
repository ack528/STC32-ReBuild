...
//  @return     void
//  @since      v1.0
//  Sample usage:               lcd_showuint32(0,0,x,5);//x为uint32类型，显示最多5位数字
//-------------------------------------------------------------------------------------------------------------------
void lcd_showuint32(uint16 x, uint16 y, uint32 dat, uint8 num)
{
    uint8 xdata a[10];
    uint8 xdata i;
    uint8 xdata displayed = 0;  // 已显示的位数计数器

    a[0] = dat / 1000000000 % 10;
    ...
    lcd_showchar(x, y*16, ' ');  // 在数值前显示一个空格

    for(i = 0; i < 10; i++)
    {
        if (a[i] == 0 && displayed == 0)  // 跳过前导零
        {
            if (i == 9) // 如果所有数字都是0
                lcd_showchar((uint8)(x+(8*i)), (uint8)(y*16), '0');
            continue;
        }
        if (displayed < num) {
            lcd_showchar((uint8)(x+(8*(displayed+1))), (uint8)(y*16), (int8)('0' + a[i]));
            displayed++;
        }
    }
}